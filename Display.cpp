...
void Display::updateUniformBufferRT(uint32_t currentImage, RayTrace rt) {
    static auto startTime = std::chrono::high_resolution_clock::now();
    ...
    //ubo.model = glm::rotate(glm::mat4(1.0f), time * glm::radians(45.0f), glm::vec3(0.0f, 0.0f, 1.0f));
    ubo.view = glm::lookAt(glm::vec3(2.0f, 2.0f, 2.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f));
    ubo.proj = glm::perspective(glm::radians(45.0f), rt.SWChainExtent.width / (float)rt.SWChainExtent.height, 0.1f, 10.0f);
    ubo.proj[1][1] *= -1;

    void* data;
    vkMapMemory(rt.device, rt.uniformBuffersMemory[currentImage], 0, sizeof(ubo), 0, &data);
    memcpy(data, &ubo, sizeof(ubo));
    vkUnmapMemory(rt.device, rt.uniformBuffersMemory[currentImage]);
}
...
void Display::drawNewFrameRT(RayTrace rt, int maxFramesInFlight, std::vector<VkFence> inFlightFences, std::vector<VkFence> imagesInFlight) {
    // Wait for the frame to be finished, with the fences
    vkWaitForFences(rt.device, 1, &inFlightFences[currentFrame], VK_TRUE, UINT64_MAX);

    // Acquire an image from the swap chain, execute the command buffer with the image attached in the framebuffer, and return to swap chain as ready to present
    uint32_t imageIndex;
    // Disable the timeout with UINT64_MAX
    VkResult res1 = vkAcquireNextImageKHR(rt.device, rt.swapChain, UINT64_MAX, rt.imageAcquiredSema[currentFrame], VK_NULL_HANDLE, &imageIndex);

    if (res1 == VK_ERROR_OUT_OF_DATE_KHR) {
        rt.recreateSwapChain(window);
        return;
    }
    else if (res1 != VK_SUCCESS && res1 != VK_SUBOPTIMAL_KHR) {
        std::_Xruntime_error("Failed to acquire a swap chain image!");
    ...
    updateUniformBufferRT(imageIndex, rt);

    // Check to make sure previous frame isnt using the image
    if (imagesInFlight[imageIndex] != VK_NULL_HANDLE) {
        vkWaitForFences(rt.device, 1, &imagesInFlight[imageIndex], VK_TRUE, UINT64_MAX);
    }
    ...
    VkSemaphore waitSemaphores[] = { rt.imageAcquiredSema[currentFrame] };
    VkPipelineStageFlags waitStages[] = { VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT };
    queueSubmitInfo.waitSemaphoreCount = 1;
    queueSubmitInfo.pWaitSemaphores = waitSemaphores;
    queueSubmitInfo.pWaitDstStageMask = waitStages;
    ...
    // Specify the command buffers to actually submit for execution
    queueSubmitInfo.commandBufferCount = 1;
    queueSubmitInfo.pCommandBuffers = &rt.commandBuffers[imageIndex];

    // Specify which semaphores to signal once command buffers have finished execution
    VkSemaphore signaledSemaphores[] = { rt.renderedSema[currentFrame] };
    queueSubmitInfo.signalSemaphoreCount = 1;
    queueSubmitInfo.pSignalSemaphores = signaledSemaphores;

    vkDeviceWaitIdle(rt.device);

    vkResetFences(rt.device, 1, &inFlightFences[currentFrame]);

    vkQueueWaitIdle(rt.graphicsQueue);

    // Finally, submit the queue info
    if (vkQueueSubmit(rt.graphicsQueue, 1, &queueSubmitInfo, inFlightFences[currentFrame]) != VK_SUCCESS) {
        std::_Xruntime_error("Failed to submit the draw command buffer to the graphics queue!");
    }
    ...
    // Specify the swap chains to present images and the image index for each chain
    VkSwapchainKHR swapChains[] = { rt.swapChain };
    presentInfo.swapchainCount = 1;
    presentInfo.pSwapchains = swapChains;
    presentInfo.pImageIndices = &imageIndex;
    ...
    VkResult res2 = vkQueuePresentKHR(rt.presentQueue, &presentInfo);
    if (res2 == VK_ERROR_OUT_OF_DATE_KHR || res2 == VK_SUBOPTIMAL_KHR) {
        rt.recreateSwapChain(window);
    }
    else if (res2 != VK_SUCCESS) {
